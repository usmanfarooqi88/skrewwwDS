...
export const contentDataComponents: ComponentDoc[] = [
  ...
  },
  {
    ...
    accessibility: "role=\"img\" with an accessible name, plus a visually-hidden (sr-only) data table with the same label/value pairs linked via aria-describedby — bar heights alone convey nothing to assistive tech. The chart is a static visualization with no keyboard tab stop.",
  ...
  },
  {
    ...
    accessibility: "role=\"img\" with an accessible name, plus a visually-hidden (sr-only) data table with the same label/value pairs linked via aria-describedby — the line path alone conveys nothing to assistive tech. The chart is a static visualization with no keyboard tab stop.",
    ...
    properties: "data ({ label, value }[]). label (accessible name / hidden table caption). height (fixed pixel height, default 240 — width is fluid, filling the parent). sparkline (boolean, default false — compact inline rendering: hides the point markers and uses a 1.5px stroke; data and accessibility are unchanged, e.g. the balance-history trend in Banking Account Card).",
  },
  {
    slug: "timeline-item",
    name: "Timeline Item",
