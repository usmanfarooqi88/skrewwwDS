...
export function Example() {
  ...
  },
  {
    ...
    figmaReference:
      "Content & Data / \"Bar Chart (example)\" (Content/Charts section, node 2058:2568), frame node 2058:2532 — 6 bars (Jan-Jun), single semantic/action/primary fill, real proportional heights (58/95/76/128/108/140 out of a 160px plot area), semantic/text/secondary Caption month labels, bar corner radius bound to radius/xs. No Y-axis, gridlines, legend, or tooltip in the Figma reference. This is a STATIC VISUAL REFERENCE frame, not a reusable Figma component (no properties, no data binding). Node IDs confirmed via direct Figma inspection 2026-07-18; re-verified 2026-09-26.",
    ...
    relatedConcepts: [],
    openQuestions: [
      "Figma covers only the single-series Bar Chart (example) static visual reference (bars + category labels); it is not a reusable Figma component. Multiple series, the value axis, grid, legend and tooltip have no Figma reference — they use current Skrewww tokens conservatively and need a design decision before parity can be claimed.",
      "Horizontal orientation, stacking modes, the value axis, grid and tooltip likewise have no Figma reference; the Figma reference bars use radius/xs, but bar spacing and the React corner radius are this implementation's own decisions.",
...
export function MultiSeriesExample() {
  ...
  },
  {
    ...
    reactLastUpdated: "2026-09-22",
    figmaReference:
      "Content & Data / \"Line Chart (example)\" (Content/Charts section, node 2058:2568), frame node 2058:2559 — single 2px semantic/action/primary stroke with round cap and join, SIX 6px hollow-ring markers (fill: semantic/surface/default, stroke: semantic/action/primary, 2px). No axis labels, gridlines, or legend in the Figma reference. This is a STATIC VISUAL REFERENCE frame, not a reusable Figma component (no properties, no data binding). Node IDs confirmed via direct Figma inspection 2026-07-18; re-verified 2026-09-26 (the earlier 7-marker note was corrected to 6).",
    figmaSourceUrl: CHARTS_FIGMA_FILE_URL,
    ...
    relatedConcepts: [],
    openQuestions: [
      "Figma covers only the single-series Line Chart (example) static visual reference (a single stroke with hollow-ring markers and no axes); it is not a reusable Figma component. Multiple series, the value axis, grid, legend and tooltip have no Figma reference — they use current Skrewww tokens conservatively and need a design decision before parity can be claimed.",
...
export function MultiSeriesExample() {
  ...
  },
  {
    ...
    reactLastUpdated: "2026-09-22",
    figmaReference:
      "Content & Data / Charts — Area Chart STATIC VISUAL REFERENCE frames (not reusable Figma components, no properties, no data binding), verified 2026-09-26: single series 3237:6726 (primary), multi-series overlap 3237:6743, stacked 3237:6769, 100% stacked 3237:6795. Guidance: 2px stroke, series color at 20% fill, category axis shown, value axis and grid hidden by default, legend for multi-series, four categorical series slots mirroring React (no success/warning/status meaning), documentation-only tooltip visual. Runtime and data behavior are React-only.",
    figmaSourceUrl: AREA_CHART_FIGMA_FILE_URL,
    figmaNodeId: AREA_CHART_FIGMA_REFERENCE_NODE_ID,
    ...
    relatedConcepts: [],
    openQuestions: [
      "A Figma visual reference now exists for Area Chart (static frames, verified 2026-09-26; not a data-bound or reusable component): 2px stroke, 20% fill, category axis shown by default, value axis and grid hidden by default, legend for multi-series, and stacking examples (overlap, stacked, 100% stacked). Runtime and data behavior remain React-only and no full Figma API parity is claimed.",
      "The categorical series palette (four slots: brand action, primary text, amber, green) is a CH-2 implementation choice checked for >= 3:1 contrast on the default white surface, not a Figma-verified palette. Slots carry no status meaning; positive/negative colors are a separate, unresolved design decision.",
      "An interactive legend (series toggling) is deferred: it needs a hidden-series state plus keyboard and announcement design. The legend is a non-interactive key.",
      "The tooltip is pointer/touch-only supplementary information; every value is also in the hidden table. A keyboard-operable chart (focusable points, arrow-key navigation) is deferred.",
      "Dense data and long category labels have no policy yet: category labels always render (no tick skipping, rotation, truncation, scrolling or aggregation), so many points or long labels can collide or clip. A density and label-length policy is an open decision.",
