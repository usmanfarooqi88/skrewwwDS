...
import { buildDistributedRegistryItems, buildValidationMessageManifest } from "@/lib/shadcn-registry-generator";
...
describe("project configuration", () => {
  ...
  it("tracks mixed Stable and Beta component maturity at platform 1.0", () => {
    ...
    expect(implemented).toHaveLength(55);
  ...
  it("preserves the supported /r install surface and Phosphor transport", () => {
    expect(readRootFile("scripts/generate-shadcn-registry.ts")).toContain("buildDistributedRegistryItems");
    expect(readRootFile("scripts/generate-shadcn-registry.ts")).toContain("buildRegistryIndex");
    expect(readRootFile("scripts/generate-shadcn-registry.ts")).toContain("registry.json");
    const names = buildDistributedRegistryItems().map((item) => item.name);
    expect(names).toContain("foundation");
    expect(names).toContain("validation-message");
    ...
    expect(names).toContain("button-group");
    expect(names).toContain("toggle-group");
    expect(names).toContain("accordion");
    expect(names).toContain("tabs");
    expect(names).toContain("search-field");
    expect(names).toContain("credit-card-field");
    expect(names).toContain("number-input");
    ...
    expect(names).toContain("menu");
    expect(names).toContain("split-button");
    expect(names).toContain("combobox");
    expect(names).toContain("select");
    expect(names).toContain("calendar-day");
    expect(names).toContain("calendar-grid");
    expect(names).toContain("date-picker");
    expect(names).toContain("phone-number-field");
