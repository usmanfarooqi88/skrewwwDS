...
describe("registry integration — Agent Kit contract and /r manifest never merge schemas", () => {
  it("public/agent/contracts/<slug>.json and public/r/<slug>.json exist as genuinely separate files with distinct shapes", () => {
    ...
    if (!existsSync(AGENT_CONTRACTS_DIR)) return; // generated by a separate script; only assert shape when present

    const buttonContract = JSON.parse(readFileSync(join(AGENT_CONTRACTS_DIR, "button.json"), "utf8")) as Record<
      string,
      unknown
    >;
