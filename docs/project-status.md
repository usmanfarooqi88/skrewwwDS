...
**Last verified: 2026-07-19** (Bar Chart / Line Chart ResponsiveContainer + curve-type corrections; see note below)
...
| TypeScript | Pass |
| Vitest | **557 tests** across **70 files** — 1 new test this pass (a regression guard in `components/ui/LineChart.test.tsx` asserting the rendered path has no curve commands); `vitest.setup.ts` gained a project-wide `ResizeObserver` polyfill, which also exposed and required fixing a pre-existing dormant bug in `useFloatingPosition.test.ts`'s own local mock |
| Playwright | **147 tests** (isolated `.next-playwright` on port 3100) — unchanged count, all re-verified passing against the corrected fluid-width/linear-curve charts |
| Production build | Pass — Turbopack (default bundler), **74/74 pages** (unchanged — no new pages, only corrected existing components), no webpack fallback needed |
