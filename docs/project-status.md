...
**Partially resolved** — Combobox confirmed 2026-07-13; File Upload confirmed 2026-07-15 (single-file anatomy; multi-file list confirmed absent); Data Table MCP verification still required
...
- File Upload component-set node ID: **`2024:2649`** ("Forms/File Upload") — confirmed via Figma MCP on 2026-07-15: 5 state variants (Empty/Dragging/Error/Disabled/Filled) + File Name text property. **Single-file trigger/state anatomy is Figma-confirmed.** The Filled variant's anatomy is a single row (File Icon + File Name + Remove Icon) with no list structure anywhere in the file — React's `multiple`/`maxFiles`/independently-removable file list is therefore **React-first with Figma parity pending** for that specific capability, not a gap in the component overall (see [`file-upload-discovery.md`](architecture/file-upload-discovery.md), `FILE_UPLOAD_MULTI_FILE_ANATOMY_STATUS = "confirmed-absent"` in `lib/file-upload-figma-metadata.ts`)
...
- File Upload multi-file list anatomy (`multiple`/`maxFiles`/independently-removable items) has no Figma spec — confirmed absent, not unaudited: the Filled variant is a single row (File Icon + File Name + Remove Icon) with no list structure anywhere in the file (see [`file-upload-discovery.md`](architecture/file-upload-discovery.md)); single-file trigger/state anatomy is otherwise Figma-confirmed
...
2. **File Upload token verification** — diff the Filled/Empty/Dragging/Error/Disabled variants' token bindings against `file-upload.module.css`'s Temporary aliases now that the component set (`2024:2649`) is confirmed; multi-file list anatomy is confirmed absent, not pending re-audit — a Figma reference frame would be needed before building toward it, same status as Combobox's option-list gap
